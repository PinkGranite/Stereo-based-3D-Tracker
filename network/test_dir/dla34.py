...
class DLA(nn.Module):
    ...
    def load_pretrained_model(self, model_dir='', data='imagenet', name='dla34', hash='ba72cf86'):
        fc = self.fc
        if model_dir:
            model_weights = torch.load(model_dir + name + '-' + hash + '.pth')
        else:
...
def dla34(pretrained, model_dir, **kwargs):  # DLA-34
    ...
    the_dla = model._modules['base']
    base_layer = the_dla._modules['base_layer']
    ...
    def __init__(self, channels, scales=(1, 2, 4, 8, 16), in_channels=None):
        ...
        for i in range(len(channels) - 1):  # 0, 1, 2
            ...
            setattr(self, 'ida_{}'.format(i),
                    # 3, [256, 128, 64], [[256, 512], [128, 256, 512], [64, 128, 256, 512]]
...
class DLASeg(nn.Module):
    def __init__(self, base_name, heads, model_dir='',
        ...
        self.base = globals()[base_name](
            pretrained, model_dir, return_levels=True)
        channels = self.base.channels  # [16, 32, 64, 128, 256, 512]
        scales = [2 ** i for i in range(len(channels[self.first_level:]))]  # [1, 2, 4, 8]
        self.dla_up = DLAUp(channels[self.first_level:], scales=scales)  # [64, 128, 256, 512], [1, 2, 4, 8]
        '''
