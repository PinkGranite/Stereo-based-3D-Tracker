...
class KittiDataset_Fusion_stereo(Dataset):
    def __init__(self, flip_rate=0., lidar_dir='lidar',
                 label_dir='label_2', calib_dir='calib', image2_dir='image_02',
                 image3_dir='image_03',
        ...
        self.dataset = kitti_object_tracking(root_dir=root_dir, split=split,
                                             lidar_dir=lidar_dir, label_dir=label_dir,
                                             calib_dir=calib_dir, image2_dir=image2_dir,
                                             image3_dir=image3_dir)
        # 700, 800
    ...
    def __getitem__(self, sequence, idx):
        shift = (np.random.rand() * 2 - 1) * self.random_shift_scale / 180 * np.pi

        # image
        image = self.dataset.get_image2(sequence, idx)
        image = self.trans(image)[None, :, :, :]
        # 这里将图像扩展至（384, 1248）方便后续处理
